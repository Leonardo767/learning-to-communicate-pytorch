...
class Arena:
    ...
    def create_step_record(self):
        ...
        if not opt.model_dial:
            record.a_comm_t = torch.zeros(
                opt.bs, opt.game_nagents, dtype=torch.long)

        # Track messages sent at time t per agent
        if opt.comm_enabled:
            comm_dtype = opt.model_dial and torch.float or torch.long
            comm_dtype = torch.float
            record.comm = torch.zeros(
                opt.bs, opt.game_nagents, opt.game_comm_bits, dtype=comm_dtype)
            if opt.model_dial and opt.model_target:
                record.comm_target = record.comm.clone()

        # Track hidden state per time t per agent
        record.hidden = torch.zeros(
            opt.game_nagents, opt.model_rnn_layers, opt.bs, opt.model_rnn_size)
    ...
    def run_episode(self, agents, train_mode=False):
        ...
        while step < episode_steps and episode.ended.sum() < opt.bs:
            ...
            for i in range(1, opt.game_nagents + 1):
                ...
                if opt.model_action_aware:
                    ...
                    for b in range(opt.bs):
                        if step > 0 and episode.step_records[step - 1].a_t[b, agent_idx] > 0:
                            prev_action[b] = episode.step_records[step -
                                                                  1].a_t[b, agent_idx]
                        if not opt.model_dial:
                            if step > 0 and episode.step_records[step - 1].a_comm_t[b, agent_idx] > 0:
                                prev_message[b] = episode.step_records[step -
                                                                       1].a_comm_t[b, agent_idx]
                    if not opt.model_dial:
                ...
                if not opt.model_dial:
                    episode.step_records[step].a_comm_t[:,
                                                        agent_idx] = comm_action
                    episode.step_records[step].q_comm_t[:,
                                                        agent_idx] = comm_value
            ...
            if step < opt.nsteps:
                for b in range(opt.bs):
                    if not episode.ended[b]:
                        ...
                        episode.r[b] = episode.r[b] + \
                            episode.step_records[step].r_t[b]
            ...
            if opt.model_target and train_mode:
                for i in range(1, opt.game_nagents + 1):
                    ...
                    if opt.comm_enabled and opt.model_dial:
                        ...
                        comm_limited = self.game.get_comm_limited(
                            step, agent.id)
                        if comm_limited is not None:
                            comm_lim = torch.zeros(
                                opt.bs, 1, opt.game_comm_bits)
                            for b in range(opt.bs):
                                if comm_limited[b].item() > 0:
                    ...
                    (action, action_value), (comm_vector, comm_action, comm_value) = \
                        agent_target.select_action_and_comm(
                            step, q_target_t, eps=0, target=True, train_mode=True)

                    # save target actions, comm, and q_a_t, q_a_max_t
                    episode.step_records[step].q_a_max_t[:,
                                                         agent_idx] = action_value
                    if opt.model_dial:
                        episode.step_records[step +
                                             1].comm_target[:, agent_idx] = comm_vector
                    else:
                        episode.step_records[step].q_comm_max_t[:,
                                                                agent_idx] = comm_value
    ...
    def average_reward(self, episode, normalized=True):
        ...
        if normalized and self.opt.game == 'switch':
