...
class CNetAgent:
    def __init__(self, opt, game, model, target, index):
        ...
        self.dru = DRU(opt.game_comm_sigma,
                       opt.model_comm_narrow, opt.game_comm_hard)
        self.id = index
        ...
        for b in range(opt.bs):
            ...
            if comm_range[b, 1].item() > 0:
                ...
                if not opt.model_dial:
                    ...
                    comm_vector[b] = self.dru.forward(
                        q[b, q_c_range], train_mode=train_mode)  # apply DRU
                ...
                for i in range(opt.game_nagents):
                    ...
                    if record.a_t[b][i].item() > 0:
                        if record.terminal[b].item() > 0:
                            ...
                            if not opt.model_dial and opt.model_avg_q:
                                q_next_max = (
                                    q_next_max + next_record.q_comm_max_t[b][i])/2.0
                    ...
                    if not opt.model_dial and record.a_comm_t[b][i].item() > 0:
                        ...
                        if record.terminal[b].item() > 0:
                            ...
                            if opt.model_avg_q:
                                q_next_max = (
                                    q_next_max + next_record.q_a_max_t[b][i])/2.0
